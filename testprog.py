...
# c.execute('CREATE TABLE IF NOT EXISTS speedAverages(longitude REAL, latitude REAL, speedavg REAL)')

# def data_entry(long, lat, list):
	# spdavg = avg(list)
	# c.execute("INSERT INTO speedAverages(longitude, latitude, speedavg) VALUES (?, ?, ?)",(long, lat, spdavg))
...
def dataEval():
	...
	# for y in range(0, totalintervals):
		# for x in range (0, totalintervals):
			# if len(grid[((-76 + (gridinterval * x)),(40 - (gridinterval * y)))]) != 0:
				# data_entry((-76 + (gridinterval * x)),(40 - (gridinterval * y)),grid[((-76 + (gridinterval * x)),(40 - (gridinterval * y)))])
